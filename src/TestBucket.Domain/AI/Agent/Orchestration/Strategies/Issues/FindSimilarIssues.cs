...
namespace TestBucket.Domain.AI.Agent.Orchestration.Strategies.Issues;
internal class FindSimilarIssues : IOrchestrationStrategy
{
    public string Name => OrchestrationStrategies.FindSimilarIssues;
    ...
    public ChatCompletionAgent[] GetMembers(Kernel kernel, ILoggerFactory loggerFactory)
    {
        return 
            [
                ...
                FindSimilarIssuesAgent.Create(kernel, loggerFactory),
                IssueGroupingAgent.Create(kernel, loggerFactory)
            ];
    }
}