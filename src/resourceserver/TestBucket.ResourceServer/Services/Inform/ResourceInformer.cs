...
/// </summary>
internal class ResourceInformer(HttpClient httpClient) : IResourceInformer
{
    ...
    {
        var appName = Assembly.GetEntryAssembly()?.GetName()?.Name ?? "ResourceServer";
        var directoryPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "testbucket", appName);
    ...
    public async Task InformAsync(TestResourceDto[] resources, CancellationToken cancellationToken)
    {
        ...
        var owner = Environment.GetEnvironmentVariable("TB_SERVER_UUID") ?? "no-uuid-configured";
