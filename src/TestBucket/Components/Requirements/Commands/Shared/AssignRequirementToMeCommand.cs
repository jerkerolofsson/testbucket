...
internal class AssignRequirementToMeCommand : ICommand
{
    public string Id => "assign-requirement-to-me";

    public string Name => _loc.Shared["assign-to-me"];
    ...
    public AssignRequirementToMeCommand(
    ...
    public async ValueTask ExecuteAsync(ClaimsPrincipal principal)
    {
        if (_appNav.State.MultiSelectedRequirements.Count > 0)
        {
            ...
            {
                await _requirementEditor.AssignRequirementToMeAsync(requirement);
            }
        }
        else
        {
